...
async def echo_message(message: Message, state: FSMContext) -> None:
    ...
    # Проверяем, не в состоянии ли пользователь поиска или админ-панели
    current_state = await state.get_state()
    
    # Если пользователь в состоянии поиска или админ-панели, не обрабатываем здесь
    # (эти состояния обрабатываются соответствующими роутерами)
    if current_state:
        state_str = str(current_state)
        if "SearchStates" in state_str or "AdminStates" in state_str:
            return
